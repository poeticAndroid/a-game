...
AFRAME.registerComponent('grabbing', {
  ...
  init() {
    ...
    this._head.glove = this._head.hand.ensure('.hitbox', 'a-sphere', {
      class: 'hitbox',
      body: 'type:kinematic;',
      radius: 0.25,
    })
  ...
  update(oldData) {
    for (let hand of this._hands) {
      let _hand = '_' + hand
      if (this[_hand].ray)
        this[_hand].ray.setAttribute(
          'raycaster',
          'far',
          this.data.grabDistance + (hand === 'head' ? 1 : 0.1875)
        )
    }
  },
  ...
  remove() {
    for (let hand of this._hands) {
      ...
      for (let finger = 0; finger < 5; finger++) {
        this.emit('fingerflex', this[_hand].glove, this[_hand].grabbed, {
          hand: hand,
          finger: finger,
          flex: flex,
        })
  ...
  tick(time, timeDelta) {
    // handle gamepads
    for (let i = 0; i < 16; i++) this._gamepadBtns[i] = 0
    ...
    if ((this._gamepadBtns[6] || this._gamepadBtns[7]) && !this._useBtn0)
      this.useDown()
    ...
    for (i = 0, len = navigator.getGamepads().length; i < len; i++) {
      ...
      if (gamepad) {
        this._grabBtn =
          this._grabBtn || this._gamepadBtns[4] || this._gamepadBtns[5]
        this._useBtn0 =
          this._useBtn0 || this._gamepadBtns[6] || this._gamepadBtns[7]
    ...
    for (let hand of this._hands) {
      ...
      if (this.data.avoidWalls && this[_hand]._occlusionRay) {
        ...
        delta.normalize()
        this[_hand]._occlusionRay.setAttribute(
          'raycaster',
          'direction',
        ...
        if (hit) {
          ...
          this[_hand].glove.object3D.position
            .copy(headPos)
            .add(palmDelta)
            .add(delta.normalize().multiplyScalar(dist - 0.03125))
          this[_hand].glove.object3D.parent.worldToLocal(
            this[_hand].glove.object3D.position
          )
        }
      }
        ...
        if (!this[_hand].grabbed.components.grabbable?.data.immovable) {
          if (this[_hand].grabbed.components.grabbable?.data.avoidWalls) {
            for (let hit of ray.intersections) {
              if (
                hit &&
                hit.el.components.wall &&
                hit.distance < -this[_hand].anchor.object3D.position.z
              ) {
                this[_hand].anchor.object3D.position.multiplyScalar(0.5)
              }
            }
          }
          let delta = THREE.Vector3.temp().copy(
      ...
      this[_hand].lastGrabbedPos =
        this[_hand].lastGrabbedPos || new THREE.Vector3()
      this[_hand].gloveVelocity =
        this[_hand].gloveVelocity || new THREE.Vector3()
      this[_hand].grabbedVelocity =
        this[_hand].grabbedVelocity || new THREE.Vector3()
      ...
      if (hand === 'head')
        this[_hand].gloveVelocity.copy(this[_hand].grabbedVelocity)
    }
  ...
  grab(hand = 'head') {
    ...
    let hit = ray.intersections[0]
    if (hit && hit.el.components.grabbable) {
      if (hand === 'head' && this.data.attractHand)
        this[_hand].ray.setAttribute('animation__pos', {
          property: 'position',
          to: { x: 0, y: -0.125, z: 0 },
          dur: 256,
        })
      ...
      if (this[_hand].grabbed.components.body != null) {
        if (!this[_hand].grabbed.components.grabbable?.data.immovable)
          this[_hand].anchor.setAttribute('joint__grab', {
            body2: this[_hand].grabbed,
            type: 'lock',
          })
      ...
      if (this[_hand].grabbed.components.grabbable.data.fixed) {
        let pos = THREE.Vector3.temp().copy(
          this[_hand].grabbed.components.grabbable.data.fixedPosition
        )
        if (hand === 'left') pos.x *= -1
        if (hand === 'head') pos.x = 0
        let quat = THREE.Quaternion.temp()
          .copy(this[_hand].ray.object3D.quaternion)
          .conjugate()
      ...
      if (
        this.data.hideOnGrab ||
        this[_hand].grabbed.components.grabbable.data.hideOnGrab
      )
      ...
      setTimeout(() => {
        let flexes = this[
          _hand
        ].grabbed.components.grabbable.data.fingerFlex.map((x) =>
          parseFloat(x)
        ) || [0.5]
        this._flexFinger(hand, 5, flexes.pop() || 0, true)
        let finger = 0
        for (let flex of flexes)
          this._flexFinger(hand, finger++, flex || 0, true)
        this.sticky = false
      }, 256)
    }
  },
  drop(hand = 'head') {
    ...
    if (this[_hand].grabbed) {
      ...
      if (
        ...
        /* ***BEGIN*** Mod for Accelerate Editor */
  ...
  useDown(hand = 'head', button = 0) {
    ...
    if (this[_hand].grabbed) {
      ...
      this.emit('usedown', this[_hand].glove, this[_hand].grabbed, {
        button: button,
      })
      this.emit(
        'use' + (button + 1) + 'down',
      ...
      this.emit('press', this[_hand].glove, this[_hand]._lastClick, {
        button: button,
      })
  ...
  useUp(hand = 'head', button = 0) {
    ...
    if (this[_hand].grabbed) {
      ...
      this.emit('useup', this[_hand].glove, this[_hand].grabbed, {
        button: button,
      })
      this.emit(
        'use' + (button + 1) + 'up',
  ...
  moveHeadHand(pz = 0, rx = 0, ry = 0, rz = 0) {
    this._head.anchor.object3D.position.z = Math.min(
      Math.max(-1.5, this._head.anchor.object3D.position.z + pz),
      -0.125
    )
  ...
  _setReticle(mode) {
    ...
    let src =
      'data:image/gif;base64,R0lGODlhEAAQAPD/AAAAAP///yH5BAUKAAIALAAAAAAQABAAAAIVlI+py+0PIwQgghDqu9lqCYbiSBoFADs='
    switch (mode) {
      case 'grab':
        src =
          'data:image/gif;base64,R0lGODlhEAAQAPD/AAAAAP///yH5BAUKAAIALAAAAAAQABAAAAI1lC8AyLkQgloMSotrVHsnhHWXdISS+DzRimIWy3Ii7CU0Tdn3mr93bvDBgMFfozg8OiaTQwEAOw=='
        break
      case 'push':
        src =
          'data:image/gif;base64,R0lGODlhEAAQAPD/AAAAAP///yH5BAUKAAIALAAAAAAQABAAAAIylA1wywIRVGMvTgrlRTltl3Wao1RmB0YVxEYqu7ZwGstWbWdcPh94O0rZgjsZEZFIagoAOw=='
        break
    }
  ...
  _enableHands() {
    ...
    for (let hand of this._hands) {
      ...
      let boxsize = 0.0625
      this[_hand].glove.ensure('.hitbox', 'a-box', {
        class: 'hitbox',
        visible: false,
        position: '0 0 0.03125',
        width: boxsize / 2,
        height: boxsize,
        depth: boxsize * 2,
      })
      ...
      this[_hand]._occlusionRay = this.el.sceneEl.ensure(
        '.occlusion-ray.' + hand,
        'a-entity',
        {
      ...
      this[_hand].ray = palm.ensure('.grabbing.ray', 'a-entity', {
        class: 'grabbing ray',
        position: hand === 'left' ? '-0.0625 0 0.0625' : '0.0625 0 0.0625',
        rotation: hand === 'left' ? '0 -45 0' : '0 45 0',
        raycaster: {
          deep: true,
          objects: '[wall], [grabbable]',
          autoRefresh: false,
      ...
      this[_hand].anchor = this[_hand].ray.ensure(
        '.grabbing.anchor',
        'a-entity',
        {
          class: 'grabbing anchor',
  ...
  _flexFinger(hand, finger, flex, priority = false) {
    ...
    if (finger < 5) {
      if (priority || !this[_hand].priorityFlex)
        this.emit('fingerflex', this[_hand].glove, this[_hand].grabbed, {
          hand: hand,
          finger: finger,
          flex: flex,
  ...
  _onButtonChanged(e) {
    ...
    switch (e.detail.id) {
      case 0: // Trigger
        finger = 1
        if (e.detail.state.pressed && !this._btnPress[hand + e.detail.id])
          this.useDown(hand)
        if (!e.detail.state.pressed && this._btnPress[hand + e.detail.id])
          this.useUp(hand)
        break
      ...
      case 3: // Thumbstick
        finger = 0
        flex = Math.max(
          this._btnFlex[hand + 3] || 0,
          this._btnFlex[hand + 4] || 0,
          this._btnFlex[hand + 5] || 0
        )
        break
      case 4: // A/X
        finger = 0
